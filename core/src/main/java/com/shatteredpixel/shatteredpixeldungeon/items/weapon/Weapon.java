...
import com.shatteredpixel.shatteredpixeldungeon.items.weapon.curses.Starving;
...
import com.shatteredpixel.shatteredpixeldungeon.items.weapon.enchantments.Divine;
...
abstract public class Weapon extends KindOfWeapon {
	...
	public Item upgrade( boolean enchant ) {
		...
		} else if (!enchant && Random.Float() > Math.pow(0.9, Math.max(0 , level() - 2))){
			enchant(null);
		}
	...
	public Item random() {
        ...
        // 33% chance of curse, if not cursed, 10% chance of enchant
        if (Random.Int(3) == 0) {
